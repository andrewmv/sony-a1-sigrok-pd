...
class Decoder(srd.Decoder):
    ...
    id = 'generic-ir'
    name = 'Generic IR'
    longname = 'Another Generic IR Decoder'
    desc = '38khz infrared'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['insignia']
    channels = (
        {'id': 'data', 'name': 'IR DATA', 'desc': 'IR Data'},
    )
    options = (
    )
    annotations = (
        ('bits', 'Bits'),
        ('command', 'Command'),
        ('command_msb', 'MSB First'),
        ('command_inv', 'Inverted'),
        ('command_msb_inv', '~MSB'),
    )
    annotation_rows = (
        ('bits', 'Bits', (0, )),
        ('data_lsb', 'Data LSB', (1, )),
        ('data_msb', 'Data MSB', (2, )),
        ('data_ilsb', 'Data ~LSB', (3, )),
        ('data_imsb', 'Data ~MSB', (4, )),
    )
        ...
        while True:
            ...
            if (bit == RC_INIT):
                ...
                if (self.bitcount >= 8):
                    ...
                    self.invbyte = self.databyte ^ 0xFF
                    self.put(self.byte_start, byte_end, self.out_ann, [1, ["0x{:02X}".format(self.databyte), "{:02X}".format(self.databyte)]])
                    self.put(self.byte_start, byte_end, self.out_ann, [2, ["0x{:02X}".format(self.reverse_bits(self.databyte)), "{:02X}".format(self.reverse_bits(self.databyte))]])
                    self.put(self.byte_start, byte_end, self.out_ann, [3, ["0x{:02X}".format(self.invbyte), "{:02X}".format(self.invbyte)]])
                    self.put(self.byte_start, byte_end, self.out_ann, [4, ["0x{:02X}".format(self.reverse_bits(self.invbyte)), "{:02X}".format(self.reverse_bits(self.invbyte))]])
                    self.bitcount = 0
                    self.databyte = 0
